...
Vlanya Call is currently sending a processed microphone track, a raw microphone
...
Route state is relayed from child frames so embedded Vlanya Call frames can be
diagnosed from the top-level chat window.

The main process also injects a small page-context fallback patch into allowed
frames and guest webviews. This catches Vlanya Call contexts where the normal
...
Rejoin the call after changing the mode so Vlanya Chat captures the microphone
again.

The chat UI customizations live in the Vlanya Chat Web build served from the
...
self-update. Use the `Vlanya-Chat-Windows-Setup-*.exe` installer when
...
`Vlanya-Chat-windows` workflow artifact.
